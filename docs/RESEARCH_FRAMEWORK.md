...
- **Simplification pass** — rejected a separate "research currency" per category (one point pool + per-node costs express specialisation without wallet sprawl); timed research **ruled out by Project Owner (DR-005, 2026-07-06): unlocks are instant, permanently** — `completionTimeMs` stays in the schema at zero, documented as unused; rejected engine-level path locking (prerequisites express structure; exclusivity waits for its future flag). ✔
