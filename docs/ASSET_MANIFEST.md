...
**Update (2026-07-12): the render *style* is now decided, the render *pipeline* still isn't built.** `docs/TECHNOLOGY_DECISION.md`'s 2026-07-12 amendment locks in flat toon/cel-shading + a bold chunky VFX/UI language (8 Visual Style Rules + a 32px silhouette litmus test) over chibi-3D geometry. This resolves the shading-style half of the first bullet above — every entry in this manifest should target that treatment specifically (rounded shapes, ≤2-stop flat colour, tint-not-texture status effects, outline/drop-glow not shadow, no gradients/grit/realistic smoke) — but the actual rendering pipeline (Three.js or equivalent, replacing the current 2D Canvas renderer) is still unbuilt, so frame counts/resolutions/rig specs remain open exactly as before.

This manifest reflects the shipped data layer as of 2026-07-11 (GP-FINAL locked), across two passes the same day, plus a 2026-07-12 style-decision update. It will grow again as future AF/GP modules add content, and as the rendering pipeline gets built — extend this document, don't replace it.
