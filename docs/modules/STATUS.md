...
| AF-024 | Research Framework | v1 *(save system implemented — AF-001 §8 debt paid; 15 categories + 11 node types registered; timed research flagged for owner ruling)* | Complete — spec + implementation (`src/core/save/`, `src/game/research/`): versioned slices w/ checksum/migration/quarantine/backup, tree engine w/ graph validation + hidden discoveries + refund reset; persistent research live in sandbox, reload-survival browser-verified | Aligned | 148 tests green; tree content + research screen bind at their modules | Approved 9.5/10 | **LOCKED** |
