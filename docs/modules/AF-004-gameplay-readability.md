...
- **Colour reconciliation (ruling recorded):** AF-004 names colour *families* (Green/Blue/Yellow…); locked AF-002 assigned exact tokens. Ruling: AF-004 semantics are realised through AF-002 tokens within each family — Healing = green family → `crystal.teal` (green-teal, per AF-002 "restoration, healing"); Shield = blue family → `plasma.cyan`; Loot = yellow family → `solar.gold`; Void = purple → `energy.violet`; Danger = `danger.red`; Warning = `warning.orange`; Technology = cyan/white per AF-002 player-tech law; Neutral = `energy.white`. Shield vs technology share a family and are disambiguated by shape (shield = arc grammar) per the colour-blind pairing law. **Project Owner may overrule** if pure green/pure blue as separate hues were intended — flagged in `docs/READABILITY_FRAMEWORK.md` §3.
